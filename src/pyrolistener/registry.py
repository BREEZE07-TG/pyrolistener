...
class ListenRegistry:
    ...
    def add_request(self, request: BaseListenRequest[BaseListenResponse]) -> None:
        if isinstance(request, CallbackQueryListenRequest):
            self.add_callback_query_request(request)
        elif isinstance(request, MessageListenRequest):
            self.add_message_request(request)
        elif isinstance(request, RawUpdateListenRequest):
            self.add_raw_update_request(request)
        else:
            raise TypeError(f"Unknown request type: {type(request).__name__}")

    def remove_request(self, request: BaseListenRequest[BaseListenResponse]) -> None:
        if isinstance(request, CallbackQueryListenRequest):
            self.remove_callback_query_request(request)
        elif isinstance(request, MessageListenRequest):
            self.remove_message_request(request)
        elif isinstance(request, RawUpdateListenRequest):
            self.remove_raw_update_request(request)
        else:
            raise TypeError(f"Unknown request type: {type(request).__name__}")